...
const LeadSettings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      if (isEditing) {
        await api.post(`/updateleadstatus/${editId}`, { name: formData.status_name });
        Swal.fire("Updated!", "Category updated successfully!", "success");
      } else {
        await api.post("/addleadstatus", { name: formData.status_name });
        Swal.fire("Created!", "New category added successfully!", "success");
      }
  ...
  const handleEdit = (status) => {
    ...
    setEditId(status.status_id || status._id);
    setIsEditing(true);
    setIsModalOpen(true);
            ...
            {leadStatuses.length === 0 ? (
              ...
              currentData.map((status) => (
                ...
                >
                  ...
                  <div>{status.status_id || status._id}</div>
                  <div className="relative">
                    <button
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
          {currentData.map((status) => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Code: {status._id || status.status_id}
                  </p>
                </div>
                <button
                  onClick={() => toggleDropdown(status.status_id || status._id)}
              ...
              <div className="text-sm text-gray-600">ID: {status.status_id || status._id}</div>
            </div>
          ))}
        </div>
        ...
        {filteredStatuses.length > 0 && (
          <div className="flex flex-col md:flex-row justify-between items-center mt-6">
            <div className="text-sm text-gray-600 mb-3 md:mb-0">
              ...
              {Math.min(indexOfLast, filteredStatuses.length)} of {filteredStatuses.length} entries
