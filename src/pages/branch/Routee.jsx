...
const Routee = () => {
  ...
  const [routes, setRoutes] = useState([
    { id: 1, name: "Indore", branch: "Main Branch" },
    { id: 2, name: "Bhopal", branch: "East Branch" },
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      setRoutes((prev) =>
        prev.map((route) =>
          route.id === editId ? { ...route, ...formData } : route
        )
      );
    } else {
      const newroute = {
  ...
  const handleEdit = (route) => {
    setFormData({ name: route.name, branch: route.branch });
    setEditId(route.id);
    setIsEditing(true);
    setIsModalOpen(true);
        ...
        <div className="flex-1 overflow-hidden rounded-[16px] border border-[#E3ECF7] bg-gradient-to-br from-white to-[#F6FAFF]">
          ...
          <div className="hidden md:block w-full flex-grow">
            <div className="w-full rounded-lg overflow-hidden">
              <div className="grid md:grid-cols-[1fr_1fr_1fr_auto_1fr] gap-x-4 px-6 py-4 border-b border-gray-200 text-[#4B5563]">
              ...
              <div className="pb-20">
                {routes?.length === 0 ? (
                  ...
                  routes?.map((route , index) => (
                    <div
                      key={route.id}
                      className="grid md:grid-cols-[1fr_1fr_1fr_auto_1fr] gap-x-4 px-6 py-4 border-b border-gray-200 items-center last:border-b-0"
                    >
...
//   const [isEditing, setIsEditing] = useState(false);
//   const [routes, setRoutes] = useState([
//     { id: 1, branch: "Indore Main", name: "Indore" },
//     { id: 2, branch: "Bhopal Center", name: "Bhopal" },
//   ]);

//   const [branches] = useState([
//     { id: 1, name: "Indore Main" },
//     { id: 2, name: "Bhopal Center" },
//     { id: 3, name: "Ujjain Branch" },
//   ]);
...
//                     >
//                       <div className="text-sm text-[#4B5563]">{route.branch}</div>
//                       <div className="text-sm text-[#4B5563]">{route.id}</div>                      
//                       <div className="text-sm text-[#4B5563]">{route.name}</div>
//                       <div className="relative">
//                         <button
//                           onClick={() => toggleDropdown(route.id)}
//                           className="p-2 text-[#4B5563] hover:bg-[#F1F5FB] rounded-full transition-colors"
